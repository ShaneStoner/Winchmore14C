...
knitr::opts_knit$set(root.dir = '/Users/shane/14Constraint Dropbox/Shane Stoner/IMPRS/NZ')
```

```{r, Load Section, results=FALSE, include=TRUE, echo=FALSE, fig.cap="The Inputs"}
...
load('./kTurnover14C.Rdata')
load('./SteadyFrame.Rdata')
...
#Input data, supplied by Troy Baisden
allInputs <- read.csv('/Users/shane/14Constraint Dropbox/Shane Stoner/IMPRS/NZ/InputsTime_TBdata.csv')
allInputs$Trial <- factor(allInputs$Trial, levels = c('Unfert', 'ResFert', 'HighFert', 'Dry', 'Irr10', 'Irr20')) #Plot in order
...
soilMass <- 0.075 * 10000 * 1.14 #Depth (cm) * area (1 ha) * bulk density (1.14 g/cm3)
alphaVal = 120 #Color transparency for some plots

#Data frame with all trial mean above ground production (Reported by Schipper et al. 2013)
InFrame <- data.frame(
...
#Let's check it out
ggplot(InFrame, aes(x = Year, y = Input, color = Trial)) + geom_line(size = 1) + ggtitle('Annual aboveground production data')
...
steadyMod <- function(k = NULL, YOI = 2005, t = NULL, plotOut = TRUE, outLength = 6, startYear = -5000, title = NULL){
    ...
    if(plotOut == TRUE){
      plot(preBombYears, preModC$preBomb14C,ylab = expression(paste(Delta^"14","C")), 
           xlab = 'Years', xlim = c(YOI - 45, YOI + 5), 
           ylim = c(-50, max(preBombClip$Delta14C)),
           type = 'l', lwd = 3, main = title)
      lines(preBombClip$Year.AD, preBombClip$Delta14C, col = 'red',lwd = 2, type = 'l')
...
# Cwindow: Runs linear, log, and polynomial regressions of data and uses fit with
# minimum error to estimate time 0 C content. This prevents short-term trends and/or 
# variation in the data from anchoring the first point and causing strange fits.
...
#writeLines(c(""), txtname) #Create a log file. Especially useful for parallel modeling, since all can write to the same .txt but not to the terminal.

roots = FALSE #Whether to use above ground inputs (roots only)
rootmod = 0.70 #Proportion of roots in top 20 cm located in top 7.5 cm
AGmod = 0.29 #Proportion of above ground production considered as inputs to soil system
lags = 1 #1 year lag from atmosphere to inputs (plants to soil(pool 1))
...
# Total iterations
niter <- 10000/shorter 
# Number of runs to toss out, for stabilization
burnin <- 5000/shorter 
# Frequently re-calculated covariance to improve jumps
updatecov <- 10
# Number of successful parameter combinations for which to calculate transit time, system age, and parameter variability
outLength <- 1000/shorter 
# Number of iterations for initial pseudo-random parameter fit. 
# Note, this step can take a very long time but usually produces 
# very good fits if this number is sufficiently high
modNum <- 5000/shorter 
...
# This loop is for each pair of start and end points - won't be used here
#for(x in xs){
...
#Selects start and end date (commented out for demonstration of single run)
#trialStart <- starts[x]
#trialEnd <- ends[x]
...
if(trial == 'Res. Fert'){
  ...
  dat = data.frame(time = c(Winch14C$year, trialEnd), soilc14 = datresfert, err = c(Winch14C$res_err, NA))
  ...
  datresfert <- dat <- dat[-c(4,6,8,10,14),]
  soilC <- soilC[-c(1,2),]
  soilCres <- soilC
...
if(trial == 'Unfert'){
  winchFertC <- read.csv('./WinchFertC_full.csv', sep = ',')
  soilC <- data.frame(time = winchFertC$year, soilC = ((winchFertC$unfert / 100) *soilMass), err =((winchFertC$unC_err / 100)*soilMass))

  #Manual data input
  datunfert<- c(Winch14C$f_con, NA)

  dat = data.frame(time = c(Winch14C$year, trialEnd), soilc14 = datunfert, err = c(Winch14C$con_err, NA))
...
if(trial == 'Irr. 10'){
  ...
  #Manual data input
  time10 = dattime =c(1949,1959, 1961, 1967, 1971, 1975, 1980, 1986, 1991, 1997, 2002, 2003)
